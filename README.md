...
## Patt92 ROCm / Strix Halo fork — V7

This branch is based on upstream llama.cpp commit
[`9f0d017efb4a388bd5c60a27a575c90f20868e51`](https://github.com/ggml-org/llama.cpp/commit/9f0d017efb4a388bd5c60a27a575c90f20868e51).
...
- Resolves fused operations per layer and device instead of disabling an entire graph
  ...
  not CPU work.
- Adds a HIP GPU kernel for rank-1 MoE LoRA-B `MUL_MAT_ID` tensors, as used by
  DeepSeek-V4 expert adapters. It replaces the invalid even-K MMVF dispatch without
  falling back to the synchronized generic route, so decode and small speculative
  verification batches retain GPU execution and graph reuse. Larger batches retain
  the safe existing fallback.
