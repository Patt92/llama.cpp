...
Based on upstream llama.cpp commit [`5cdd3d1dad5cbb7107b3e9f6d23239ba88ac0123`](https://github.com/ggml-org/llama.cpp/commit/5cdd3d1dad5cbb7107b3e9f6d23239ba88ac0123).
...
- Lets a shared MTP draft borrow `token_embd` and `output` from its target. Unsloth's `mtp-Qwen3.8-Flash-Next-shared-*.gguf` ships the NextN block alone; the loader required `token_embd` unconditionally and failed with `check_tensor_dims: tensor 'token_embd.weight' not found`. Both are now optional when the trunk is absent and are resolved through `ctx_other` at graph time, with a shape check against the target. Only usable as a draft (`-md`), never standalone.
- Treats only `gemma4-assistant` as memory-shared in speculative decoding. The flag skips the catch-up decode and places every draft token at the same position, which is the Gemma4-assistant convention; it was being applied to every MTP draft that pointed at its target, including Qwen3.8-Flash-Next, whose NextN head keeps its own KV and needs sequential positions. Follows [`#28243`](https://github.com/ggml-org/llama.cpp/pull/28243), whose author reports 1.3-2x faster MTP from this; not yet measured here.
- Gathers the QSA-selected keys and values into a compact buffer for decode, instead of masking them out of a full-width attention. The indexer already picked roughly 2048 positions, but expressing that as a mask over the whole cache left the cost growing with context depth. Taken from [`#28213`](https://github.com/ggml-org/llama.cpp/pull/28213), which limited it to one token per stream; its author measured +6% at 31k, +19% at 62k and +50% at 130k with byte-identical greedy output. Extended here to ubatches of up to 8 tokens per stream, because speculative verification hands the target `n-max + 1` tokens per pass and the one-token gate sent every such pass down the masked path -- with `--spec-draft-n-max 2` the gather never engaged at all, which is why Qwen3.8-Flash-Next decoded at 30 t/s at 15k but 20 t/s past 50k. Each token carries its own top-k list and becomes one entry of the attention batch; `test-llama-archs --arch qwen4exp` checks an 8-token verify batch against eight single-token gathers (NMSE 0 on gfx1151). Engages from about 9k of context; `QWEN4EXP_QSA_GATHER=0` disables it, `=2` forces it. Note this does nothing unless the GGUF carries non-zero `attention.compress_ratios` -- everything on that path hangs off the same flag.
...
cd llama.cpp
git checkout 5cdd3d1dad5cbb7107b3e9f6d23239ba88ac0123
