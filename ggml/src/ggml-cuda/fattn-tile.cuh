...
// [TAG_FATTN_TILE_SPARSE] __ballot_sync has never been compiled under HIP in this tree: the only
// other use sits inside a !GGML_USE_HIP guard. Rather than rely on a compatibility shim and on a
// 64-bit mask literal meaning the right thing on a wave32 part, name the intrinsic per backend.
// HIP's __ballot always returns 64 bits with only the live lanes set, which is what the popcll /
// ffsll below expect on both wave32 and wave64.
static __device__ __forceinline__ uint64_t ggml_cuda_fattn_ballot(bool pred) {
#ifdef GGML_USE_HIP
...
#pragma unroll
        for (int i_KQ_0 = 0; i_KQ_0 < nbatch_fa; i_KQ_0 += np*warp_size) {
            ...
            if (i_KQ < k_VKQ_sup) {
                if constexpr (direct_sparse_K) {
                    ggml_cuda_memcpy_1<cpy_nb>(
                        &K_k[i_KQ_0/(np*warp_size)], K_h2 + int64_t(k_VKQ_0 + i_KQ)*stride_K2 + k_KQ_0/2 + k_KQ_1);
                } else {
...
#pragma unroll
    for (int i_KQ_0 = 0; i_KQ_0 < nbatch_fa; i_KQ_0 += np*warp_size) {
        ...
        i_KQ_map[i_KQ_0/(np*warp_size)] = i_KQ;
        ...
        if constexpr (compact_mask) {
            if (mask) {
                ...
                uint64_t selected_lanes = ggml_cuda_fattn_ballot(selected);
...
#pragma unroll
    for (int k_KQ_0 = 0; k_KQ_0 < DKQ - nbatch_K_last; k_KQ_0 += nbatch_K) {
        flash_attn_tile_iter_KQ<warp_size, nwarps, ncols1, ncols2, DKQ, nbatch_fa, nbatch_K, use_logit_softcap, oob_check, T_vec_dot, type_K>(
            Q_tmp, K_h2, K_c, KV_tmp, stride_K2, stride_K_bytes, k_VKQ_0, k_VKQ_sup, k_KQ_0, i_KQ_map, KQ_acc);
    }
...
#pragma unroll
            for (int i0 = 0; i0 < nbatch_fa; i0 += np*warp_size) {
                ...
                const float val = (!compact_mask && !oob_check) || i_KQ < k_VKQ_sup ?
...
#pragma unroll
        for (int i0 = 0; i0 < nbatch_fa; i0 += np*warp_size) {
            ...
            if (!compact_mask || i_KQ < k_VKQ_sup) {
...
#pragma unroll
                for (int i0 = 0; i0 < DVp/2; i0 += warp_size*cpy_ne_D) {
                    ggml_cuda_memcpy_1<cpy_ne_D*4>(
                        &V_k[i0/warp_size], V_h2 + int64_t(k_VKQ_0 + i_KQ)*stride_V2 + i0 + threadIdx.x*cpy_ne_D);
                }
    ...
    if (ncols2 == 1) {
        ...
        while (k_VKQ_0 < k_VKQ_max - nbatch_fa) {
            ...
            flash_attn_tile_iter<warp_size, nwarps, ncols1, ncols2, DKQ, DV, nbatch_fa, nbatch_K, use_logit_softcap, oob_check, type_K, type_V>
                ...
                stride_K2, stride_V2, stride_K_bytes, stride_V_bytes, stride_mask, KQ_max, KQ_sum, VKQ, k_VKQ_0, k_VKQ_max, col_Q_0);
    ...
    if constexpr (DKQ <= 128) {
        if (Q->ne[1] > 32/ncols2) {
            ...
            launch_fattn<DV, cols_per_block/ncols2, ncols2>
                (ctx, dst, fattn_kernel, nwarps, nbytes_shared, nbatch_fa, type_K == GGML_TYPE_F16, type_V == GGML_TYPE_F16, false, /*use_sparse =*/ false, warp_size);
            return;
        }
    }
#endif // GGML_USE_HIP
