...
static bool ggml_cuda_mul_mat_id_needs_sync(const ggml_tensor * dst, const int cc) {
    ...
    if (GGML_CUDA_CC_IS_AMD(cc) && ggml_cuda_should_use_mmvf_rank1_id(src0, src1, dst->src[2], dst)) {
        return false;
    }
    ...
    if (dst->ne[2] <= MMVQ_MAX_BATCH_SIZE) {
        ...
        } else if (GGML_CUDA_CC_IS_AMD(cc) && ggml_cuda_should_use_mmvf(src0->type, cc, src0->ne, src0->nb, src1->ne[2])) {
            return false;
        }
    }
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // [TAG_MUL_MAT_ID_CUDA_GRAPHS]
    if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        if (GGML_CUDA_CC_IS_AMD(cc) && ggml_cuda_should_use_mmvf_rank1_id(src0, src1, ids, dst)) {
            ggml_cuda_mul_mat_vec_f_rank1_id(ctx, src0, src1, ids, dst);
            return;
        }
        ...
        if (ne2 <= MMVQ_MAX_BATCH_SIZE) {
            if (ggml_is_quantized(src0->type)) {
                ...
                if (GGML_CUDA_CC_IS_AMD(cc) && ggml_cuda_should_use_mmvf(src0->type, cc, src0->ne, src0->nb, ne12)) {
