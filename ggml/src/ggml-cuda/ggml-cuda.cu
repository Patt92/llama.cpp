...
static const void * ggml_cuda_graph_get_key(ggml_cgraph * cgraph) {
    // [TAG_CUDA_GRAPH_KEY_SHAPE] the first node alone is a poor key when one context alternates
    // between graphs of different token counts, which speculative decoding does all the time: the
    // draft context decodes 1 token per draft step and then catches up on the accepted tokens
    // (2..n) with the same first node, so every switch found the other shape's properties in the
    // slot, reset the warmup and captured again. On gfx1151 that made HIP graphs a net loss
    // (capture 20% of the graph computes). Fold the node count and the token dimension of the first
    // node into the key so each shape keeps its own captured graph.
    const ggml_tensor * n0 = cgraph->nodes[0];
    uintptr_t key = (uintptr_t) n0;
    key ^= (uintptr_t) cgraph->n_nodes * (uintptr_t) 0x9E3779B97F4A7C15ull;
    key ^= (uintptr_t) n0->ne[1] << 40;
    key ^= (uintptr_t) n0->ne[2] << 52;
...
static int ggml_cuda_try_fuse(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, int i) {
    ...
    // [TAG_MMQ_ID_PAIR] two consecutive MUL_MAT_ID nodes that share the activation and the ids
    // (the gate/up pair of a MoE FFN) can share one ids-helper launch and one q8_1 quantization
    // of src[1]. The two matmuls themselves stay separate MMQ launches with unchanged arguments,
    // so this batches work without fusing any math. Taken from myhacsint/llama.cpp
