...
void ggml_cuda_mul_mat_vec_q(
    ...
    // [TAG_MMVQ_Q8_1_CACHE] within one graph evaluation the same activation is quantized once
    // per expert matmul; ask the context for a buffer it may already hold. Returns nullptr when
    // the cache is inactive, in which case we fall back to a per-call allocation.
