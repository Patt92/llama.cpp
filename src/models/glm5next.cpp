...
void llama_model_glm5next::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer_all; ++i) {
        ...
        if (is_mtp) {
            ...
            // the NextN block may share the trunk's embeddings, LM head and output norm
            // instead of carrying its own; graph_mtp falls back to model.tok_embd,
            // model.output and model.output_norm when these are absent
...
llm_graph_input_kpool * llama_model_glm5next::graph::build_inp_kpool(llm_graph_input_mem_hybrid_kpool * inp_hyb) {
...
ggml_tensor * llama_model_glm5next::graph::build_mla_layer(
    ...
    // Keep the sparse mask construction local to GLM5Next. The generic upstream
    // K-only attention path remains untouched for every other architecture.
    ggml_build_forward_expand(gf, Qcur);
    ggml_build_forward_expand(gf, kv_cmpr);
    ggml_build_forward_expand(gf, inp_attn->mctx->cpy_k(ctx0, kv_cmpr, inp_attn->get_k_idxs(), il));
    ...
    if (top_k) {
        ...
        ggml_tensor * zeros = ggml_fill(ctx0,
                ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, 1, top_k_3d->ne[0], top_k_3d->ne[1], top_k_3d->ne[2]), 0.0f);
        ggml_tensor * sparse = ggml_set_rows(ctx0, mask_all, zeros, top_k_3d);
        sparse = ggml_view_4d(ctx0, sparse, sparse->ne[1], sparse->ne[2], 1, sparse->ne[3],
                sparse->nb[2], sparse->nb[3], sparse->nb[3], 0);
        kq_mask = ggml_add(ctx0, sparse, kq_mask);
        n_kv_max = top_k->ne[0];
    }
    ...
    // The indexer's selection is not handed to the attention node: the HIP sparse kernels were
    // reverted after they faulted on Ornith in the K/V f16 scratch buffer. The API and the
    // plumbing stay; nothing passes indices until that is understood.
...
llama_model_glm5next::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    auto * inp_hyb  = build_inp_mem_hybrid_kpool();
