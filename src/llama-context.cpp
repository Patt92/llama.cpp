...
static int64_t g_nan_hunt_n_inf  = 0; // Inf count of the last scan, reported next to the NaN count
static int64_t g_nan_hunt_n_pinf = 0; // of which +Inf: masks hold only -inf, overflows usually both
    ...
    if (t->type == GGML_TYPE_F32) {
        ...
        for (int64_t i = 0; i < n; i += step) {
            if (std::isnan(d[i])) {
                if (first < 0) { first = i; }
                n_nan++;
            } else if (std::isinf(d[i])) {
                g_nan_hunt_n_inf++;
                g_nan_hunt_n_pinf += d[i] > 0;
            }
        }
    } else {
...
static bool llama_context_nan_hunt_cb(struct ggml_tensor * t, bool ask, void * user_data) {
    ...
    if (first < 0) {
        ...
        // the graph is walked in topological order, so the first Inf-carrying tensors that
        // are not masks or biased scores are the birth region; report them with their sources
        const std::string name = ggml_get_name(t);
        const bool view_like = t->op == GGML_OP_CONT || t->op == GGML_OP_RESHAPE || t->op == GGML_OP_VIEW ||
            t->op == GGML_OP_PERMUTE || t->op == GGML_OP_TRANSPOSE || t->op == GGML_OP_CPY || t->op == GGML_OP_FILL;
        if (g_nan_hunt_n_inf == 0 || t->op == GGML_OP_NONE || (view_like && g_nan_hunt_n_pinf == 0) ||
        ...
        for (int j = 0; j < GGML_MAX_SRC; ++j) {
            ...
            const std::string sname = ggml_get_name(src);
            if (sname.find("mask") != std::string::npos || sname.find("kq") != std::string::npos ||
                sname.find("score") != std::string::npos || sname.find("bias") != std::string::npos ||
                sname.find("indexer") != std::string::npos || sname.find("leaf") != std::string::npos) {
                return true; // consumes a mask or a biased score: -inf is expected here
            }
        }
        inf_born = true;
    ...
    const llama_ubatch * ub = g_nan_hunt_ubatch;
    if (ub != nullptr && ub->token != nullptr && t->ne[1] == (int64_t) ub->n_tokens && t->ne[2] == 1) {
        std::string cols;
        for (int64_t c = std::max<int64_t>(0, i1 - 2); c < std::min<int64_t>(ub->n_tokens, i1 + 8); ++c) {
            cols += format(" [%" PRId64 "] tok=%d pos=%d", c, ub->token[c], ub->pos[c]);
        }
        LLAMA_LOG_ERROR("[TAG_NAN_CHECK]   columns around the first NaN:%s\n", cols.c_str());
    }
    ...
    for (int j = 0; j < GGML_MAX_SRC; ++j) {
        ...
        LLAMA_LOG_ERROR("[TAG_NAN_CHECK]   src%d '%s' op=%s type=%s ne=[%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 "] n_nan=%" PRId64 " n_inf=%" PRId64 " first=%" PRId64 "\n",
                j, ggml_get_name(src), ggml_op_desc(src), ggml_type_name(src->type), src->ne[0], src->ne[1], src->ne[2], src->ne[3], s_nan, s_nan >= 0 ? g_nan_hunt_n_inf : (int64_t) -2, s_first);
    }
...
llama_context::llama_context(
    ...
    // [TAG_QWEN4EXP_SHARED_MTP] a shared qwen4exp draft borrows token_embd / output from its target
    // through ctx_other, the same way EAGLE3 and DFLASH drafts do. A qwen4exp model that carries
    // both (the full model, the self-contained draft export) keeps ctx_other null as before.
